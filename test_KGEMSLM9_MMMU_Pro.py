...
parser.add_argument("--image-folder", type=str, default="/home/gtl/LLaVA-main/playground/data/MMMU_Pro_img/")
parser.add_argument("--question-file", type=str, default="./playground/data/mmmu_pro_questions.json")
...
def KGEM_2(image_tensor,image_size,in_text,in_graph,out_think):
    ...
    conv.system = f"A conversation between a curious human and an artificial intelligence assistant. {out_think}. {in_graph}. According to the user's input and the assistant's thoughts, answer the user's question with A/B/C/D/E. only reply A/B/C/D/E"#系统提示词
    roles = conv.roles
...
f=open('dataset_output_mmmu_pro.json', 'r', encoding='utf-8') 
alldata = json.load(f)
...
# 保存dataset为json文件
with open('dataset_output_mmmu_pro2.json', 'w', encoding='utf-8') as f:
